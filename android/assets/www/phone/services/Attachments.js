(function(window, angular, undefined) {
    ...
    angular.module("mgApp").factory("attachments", function($rootScope) {
        function noop() {}
        ...
        Attachment.prototype.remotePath =
            ...
            ($fh.cloud_props ? $fh.cloud_props.hosts.releaseCloudUrl : "https://maygurney-h1txnhqhdxurtj9mxk6vsibe-live_maygurney.df.live.e111.feedhenry.net").replace(/_\w+/g, "") + "/cloud/";

        /**
         * @return {String} The url to upload an attachment to.
         */
        Attachment.prototype.getUploadPath = function() {
            return this.remotePath + "uploadPhoto";
        };

        /**
         * @return {String} The url to download an attachment from.
         */
        Attachment.prototype.getDownloadPath = function() {
            return this.remotePath + "downloadPhoto?identifier=" + this.identifier;
        };
        ...
        Attachment.prototype.download = function(cb) {
            ...
            function download() {
                ...
                fileTransfer.download(encodeURI(this.getDownloadPath()), this.getLocalPath(), function(fileEntry) {
        ...
        Attachment.prototype.upload = function(cb) {
            ...
            if(this.remoteStatus === this.EXISTS || this.remoteStatus === this.INPROGRESS) {
                ...
                _updateProgress();
                fileTransfer.upload(this.getLocalPath(), this.getUploadPath(), function() {
        ...
        function create(fileUrl, cb) {
            ...
            window.plugins.fileHasher.hash(fullPath, function(err, hash) {
                if(err) {
                    console.log("error from file hasher: " + JSON.stringify(err, null, 4));
                    return cb(err);
                }
                ...
                fileSystem.root.getFile(fullPath, null, function(fileEntry) {
                    ...
                    if(path.join(fileSystem.root.fullPath, hash + extension) === fileEntry.fullPath) {
                        ...
                        fileEntry.moveTo(fileSystem.root, hash + extension, finish, cb);
                    }
